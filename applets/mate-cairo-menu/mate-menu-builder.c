...
MateMenuTree *  main_menu_tree = NULL;
MateMenuTree *  settings_menu_tree = NULL;
...
_fill_session_menu (GtkWidget * menu)
{
  ...
  if (have_mate_session_manager)
  {
    add_special_item (menu,_("Logout"),"mate-session-save","--logout-dialog --gui","mate-logout",NULL);
  }
  else if (dbus_service_exists ("org.xfce.SessionManager") )
  {
    add_special_item (menu,_("Logout"),"xfce4-session-logout","","mate-logout",NULL);
  }
  else
  {
    /* hope that mate-session-save exists; needed for GNOME 2.22, at least. */
    add_special_item (menu, _("Logout"), "mate-session-save",
                      "--kill --gui", "mate-logout",NULL);
  }
  if (dbus_service_exists ("org.mate.ScreenSaver"))
  {
    if (!add_special_item (menu,_("Lock Screen"),"mate-screensaver-command","--lock","mate-lockscreen",NULL))
    {
      add_special_item (menu,_("Lock Screen"),"dbus-send","--session --dest=org.mate.ScreenSaver --type=method_call --print-reply --reply-timeout=2000 /org/mate/ScreenSaver org.mate.ScreenSaver.Lock","mate-lockscreen",NULL);
    }
  }
  else
  {
  ...
  {
    if (!add_special_item (menu,_("Suspend"),"mate-power-cmd","suspend","mate-session-suspend",NULL))
    {
      add_special_item (menu,_("Suspend"),"dbus-send","--session --dest=org.freedesktop.PowerManagement --type=method_call --print-reply --reply-timeout=2000 /org/freedesktop/PowerManagement org.freedesktop.PowerManagement.Suspend","mate-session-suspend",NULL);
    }

    if (!add_special_item (menu,_("Hibernate"),"mate-power-cmd","hibernate","mate-session-hibernate",NULL))
    {
      add_special_item (menu,_("Hibernate"),"dbus-send","--session --dest=org.freedesktop.PowerManagement --type=method_call --print-reply --reply-timeout=2000 /org/freedesktop/PowerManagement org.freedesktop.PowerManagement.Hibernate","mate-session-hibernate",NULL);
    }

  }
  else if (dbus_service_exists ("org.mate.PowerManagement"))
  {
    if (!add_special_item (menu,_("Suspend"),"mate-power-cmd","suspend","mate-session-suspend",NULL))
    {
  ...
  if (have_mate_session_manager)
  {
    add_special_item (menu,_("Shutdown"),"mate-session-save","--shutdown-dialog --gui","mate-shutdown","mate-logout",NULL);
  }
...
_get_places_menu (GtkWidget * menu)
{
  ...
  if (g_list_length (volumes) > MAX_ITEMS_OR_SUBMENU)
  {
    ...
    image = get_gtk_image ("mate-dev-removable");
    gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (item),image);
...
static GtkWidget *
fill_er_up(MenuInstance * instance,MateMenuTreeDirectory *directory, GtkWidget * menu)
{
  ...
  {
    sanity_depth_count--;
    g_warning ("%s: Exceeded max menu depth of 6 at %s",__func__,matemenu_tree_directory_get_name((MateMenuTreeDirectory*)directory));
  ...
  while (tmp != NULL)
  {
    ...
    switch (matemenu_tree_item_get_type(item))
    {
      ...
      case MATEMENU_TREE_ITEM_ENTRY:
        entry = NULL;
        if (matemenu_tree_entry_get_is_excluded ((MateMenuTreeEntry *) item))
        {
          break;
        }
        if (matemenu_tree_entry_get_is_nodisplay ((MateMenuTreeEntry *) item))
        {
          break;
        }
        txt = matemenu_tree_entry_get_name( (MateMenuTreeEntry*)item);
        desktop_file = g_strdup (matemenu_tree_entry_get_desktop_file_path ((MateMenuTreeEntry*)item));
...
//          g_signal_connect(G_OBJECT(menu_item), "button-release-event", G_CALLBACK(_launch), desktop_file);
      ...
      case MATEMENU_TREE_ITEM_DIRECTORY:
        if (!matemenu_tree_directory_get_is_nodisplay ( (MateMenuTreeDirectory *) item) )
        {
          CallbackContainer * c;
          gchar * drop_data;
          c = g_malloc0 (sizeof(CallbackContainer));
          c->icon_name = g_strdup(matemenu_tree_directory_get_icon ((MateMenuTreeDirectory *)item));
          image = get_gtk_image (c->icon_name);
          if (!image)
          {
            image = get_gtk_image ("stock_folder");
          }
          sub_menu = GTK_WIDGET(fill_er_up( instance,(MateMenuTreeDirectory*)item,NULL));
          txt = matemenu_tree_directory_get_name((MateMenuTreeDirectory*)item);
          menu_item = cairo_menu_item_new_with_label (txt?txt:"unknown");
          ...
          gtk_menu_shell_append(GTK_MENU_SHELL(menu),menu_item);
          c->file_path = g_strdup(matemenu_tree_directory_get_desktop_file_path ((MateMenuTreeDirectory*)item));
          c->display_name = g_strdup (matemenu_tree_directory_get_name ((MateMenuTreeDirectory*)item));
          c->instance = instance;
          /*
...
//    printf("MATEMENU_TREE_ITEM_HEADER\n");
        break;
...
//    printf("MATEMENU_TREE_ITEM_ALIAS\n");
        /*      {
...
static void
_menu_modified_cb(MateMenuTree *tree,MenuInstance * instance)
{
...
find_menu_dir (MenuInstance * instance, MateMenuTreeDirectory * root)
{
  g_return_val_if_fail (root,NULL);
  GSList * items = NULL;
  GSList * tmp;
  MateMenuTreeDirectory * result = NULL;
  const gchar * txt = NULL;

  txt = matemenu_tree_directory_get_desktop_file_path (root);
  if (g_strcmp0(txt,instance->submenu_name)==0 )
  {
    matemenu_tree_item_ref (root);
    return root;
  }
  ...
  while (tmp != NULL)
  {
    ...
    switch (matemenu_tree_item_get_type(item))
    {
      case MATEMENU_TREE_ITEM_DIRECTORY:
        if (!matemenu_tree_directory_get_is_nodisplay ( (MateMenuTreeDirectory *) item) )
        {
          txt = matemenu_tree_directory_get_desktop_file_path ((MateMenuTreeDirectory*)item);
          if (g_strcmp0(txt,instance->submenu_name)==0 )
          {
            result = (MateMenuTreeDirectory*)item;
            break;
          }
          else if (!result)  /*we're continuing looping if result to unref the remaining items*/
          {
            result = find_menu_dir (instance, (MateMenuTreeDirectory *) item);
          }
        }
...
static void
_submenu_modified_cb(MateMenuTree *tree,MenuInstance * instance)
{
...
{
  g_debug ("%s",__func__);
  MateMenuTreeDirectory *main_root;
  matemenu_tree_remove_monitor (main_menu_tree,(MateMenuTreeChangedFunc)_submenu_modified_cb,instance);
}
...
_remove_settings_submenu_cb(MenuInstance * instance,GObject *where_the_object_was)
{
  g_debug ("%s",__func__);
  MateMenuTreeDirectory *main_root;
  matemenu_tree_remove_monitor (settings_menu_tree,(MateMenuTreeChangedFunc)_submenu_modified_cb,instance);
}
...
{
  MateMenuTreeDirectory *main_root;
  MateMenuTreeDirectory *settings_root;
  GtkWidget * menu = NULL;
  /*
  ...
  if (!main_menu_tree)
  {
    main_menu_tree = matemenu_tree_lookup("applications.menu", MATEMENU_TREE_FLAGS_NONE);
  }
  if (!settings_menu_tree)
  {
    settings_menu_tree = matemenu_tree_lookup("settings.menu", MATEMENU_TREE_FLAGS_NONE);
  }
  ...
  {
    MateMenuTreeDirectory * menu_dir = NULL;

    main_root = matemenu_tree_get_root_directory(main_menu_tree);
    g_assert (matemenu_tree_item_get_type( (MateMenuTreeItem*)main_root) == MATEMENU_TREE_ITEM_DIRECTORY);
    g_assert (main_root);
    settings_root = matemenu_tree_get_root_directory(settings_menu_tree);
    if ( menu_dir = find_menu_dir (instance,main_root) )
    {
      /* if instance->menu then we're refreshing in a monitor callback*/
      matemenu_tree_remove_monitor (main_menu_tree,(MateMenuTreeChangedFunc)_submenu_modified_cb,instance);
      matemenu_tree_add_monitor (main_menu_tree,(MateMenuTreeChangedFunc)_submenu_modified_cb,instance);
    ...
    if (menu_dir)
    {
      matemenu_tree_item_unref(menu_dir);
    }
    matemenu_tree_item_unref(main_root);
    if (settings_root)
    {
      matemenu_tree_item_unref(settings_root);
    }
  }
...
menu_build (MenuInstance * instance)
{
  ...
  if (main_menu_tree)
  {
    root = matemenu_tree_get_root_directory(main_menu_tree);
    if (root)
    {
      g_assert (!instance->submenu_name);
      matemenu_tree_remove_monitor (main_menu_tree,(MateMenuTreeChangedFunc)_menu_modified_cb,instance);
      matemenu_tree_add_monitor (main_menu_tree,(MateMenuTreeChangedFunc)_menu_modified_cb,instance);
  ...
  if (settings_menu_tree)
  {
    root = matemenu_tree_get_root_directory(settings_menu_tree);
    matemenu_tree_remove_monitor (settings_menu_tree,(MateMenuTreeChangedFunc)_menu_modified_cb,instance);
    matemenu_tree_add_monitor (settings_menu_tree,(MateMenuTreeChangedFunc)_menu_modified_cb,instance);
    if (!instance->menu)
    {
    ...
    else
    {
      ...
      c = g_malloc0 (sizeof(CallbackContainer));
      c->icon_name = g_strdup(matemenu_tree_directory_get_icon (root));
      image = get_gtk_image (c->icon_name);
      txt = matemenu_tree_entry_get_name((MateMenuTreeEntry*)root);
      menu_item = cairo_menu_item_new_with_label (txt?txt:"unknown");
      gtk_menu_item_set_submenu (GTK_MENU_ITEM(menu_item),sub_menu);
      if (image)
      {
      ...
      gtk_menu_shell_append(GTK_MENU_SHELL(instance->menu),menu_item);
      c->file_path = g_strdup(matemenu_tree_directory_get_desktop_file_path (root));
