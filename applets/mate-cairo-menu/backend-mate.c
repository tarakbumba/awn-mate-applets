...
static void
add_entry(MateMenuTreeEntry *entry,
          const char     *path, GSList**p)
{
  ...
  item->name = matemenu_tree_entry_get_name(entry);
  item->icon = matemenu_tree_entry_get_icon(entry);
  item->exec = matemenu_tree_entry_get_exec(entry);
  char *str = item->exec;
  ...
  item->comment = matemenu_tree_entry_get_comment(entry);

  item->launch_in_terminal = matemenu_tree_entry_get_launch_in_terminal(entry);
...
static void
fill_er_up(MateMenuTreeDirectory *directory, GSList**p)
{
  GSList*  data = *p;
  GSList     *items;
  GSList     *tmp;
  ...
  while (tmp != NULL)
  {
    ...
    switch (matemenu_tree_item_get_type(item))
    {
      ...
      case MATEMENU_TREE_ITEM_ENTRY:
        // print_entry (MATEMENU_TREE_ENTRY (item), path);
        add_entry(MATEMENU_TREE_ENTRY(item), path, &data);
        break;
      ...
      case MATEMENU_TREE_ITEM_DIRECTORY:
      {
        ...
        dir_item->name = matemenu_tree_directory_get_name(item);
        dir_item->desktop = matemenu_tree_directory_get_desktop_file_path(item);
        dir_item->comment = NULL;
        dir_item->null = NULL;
        dir_item->comment = matemenu_tree_directory_get_comment(item);
        dir_item->icon = matemenu_tree_directory_get_icon(item);
        dir_item->sublist = NULL;
        data = g_slist_append(data, dir_item);
        fill_er_up(MATEMENU_TREE_DIRECTORY(item), &dir_item->sublist);
...
static void monitor_places(gpointer callback, gpointer data, gpointer box)
{
  ...
  if (gnome_vfs_monitor_add(&handle, filename, MATE_VFS_MONITOR_FILE,
                            monitor_places_callback, Monitor_place) != MATE_VFS_OK)
  {
...
GSList* get_menu_data(gboolean show_search, gboolean show_run, gboolean show_places, gboolean show_logout, char* file_manager, char*logout)
{
  ...
  const char * menu_file[] = {"matecc.menu", "preferences.menu", "settings.menu", NULL};//
  ...
  menu_tree = matemenu_tree_lookup("applications.menu", MATEMENU_TREE_FLAGS_NONE);

  if (menu_tree)
  {
    root = matemenu_tree_get_root_directory(menu_tree);
  ...
  menu_tree = matemenu_tree_lookup("matecc.menu", MATEMENU_TREE_FLAGS_NONE);
  ...
  if (menu_tree)
  {
    ...
    if (root)
    {
      ...
      dir_item->comment = g_strdup("Mate Control Centre");
      dir_item->null = NULL;
      dir_item->sublist = NULL;
      dir_item->icon = g_strdup("mate-control-center");
      data = g_slist_append(data, dir_item);
  ...
  menu_tree = matemenu_tree_lookup("settings.menu", MATEMENU_TREE_FLAGS_NONE);
  ...
  if (menu_tree)
  {
    ...
    if (root)
    {
      ...
      dir_item->sublist = NULL;
      dir_item->null = NULL;
      dir_item->icon = g_strdup("mate-settings");
      data = g_slist_append(data, dir_item);
  ...
  if (show_logout)
  {
    ...
    dir_item->icon = g_strdup("mate-logout");
