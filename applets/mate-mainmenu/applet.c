...
#include "config.h"
#define MATEMENU_I_KNOW_THIS_IS_UNSTABLE 1
...
#define APPLET_NAME "mate-mainmenu"
...
typedef struct {
  ...
  MateMenuTreeDirectory *apps;
  MateMenuTreeDirectory *settings;
...
static void
launch (MateMenuTreeEntry *tree_entry)
{
  const gchar *path;
  GError *error = NULL;
  ...
  path = matemenu_tree_entry_get_desktop_file_path (tree_entry);
...
static void
on_item_clicked (GtkButton *button, MateMenuTreeItem *item)
{
  switch (matemenu_tree_item_get_type (item))
  {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      menu->root = MATEMENU_TREE_DIRECTORY (item);
      populate (menu);
      break;
    case MATEMENU_TREE_ITEM_ENTRY:
        launch (MATEMENU_TREE_ENTRY (item));
      break;
    default:
      return;
  }
...
static GtkWidget*
make_item (MateMenuTreeItem *item)
{
  GtkWidget *button;
  GtkWidget *vbox;
  GtkWidget *image;
  ...
  {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      name = matemenu_tree_directory_get_name (MATEMENU_TREE_DIRECTORY (item));
      icon = matemenu_tree_directory_get_icon (MATEMENU_TREE_DIRECTORY (item));
      break;
    case MATEMENU_TREE_ITEM_ENTRY:
      name = matemenu_tree_entry_get_name (MATEMENU_TREE_ENTRY (item));
      icon = matemenu_tree_entry_get_icon (MATEMENU_TREE_ENTRY (item));
      break;
    default:
      return NULL;
  }
...
{
  MateMenuTreeDirectory *temp= menu->root;
  menu->root = matemenu_tree_item_get_parent (MATEMENU_TREE_ITEM (temp));
  if (menu->root == menu->settings)
    menu->root = matemenu_tree_get_root_directory (menu->tree);
  populate (menu);
}

static gint
_compare (MateMenuTreeItem *item1, MateMenuTreeItem *item2)
{
  const gchar *name1 = NULL;
  const gchar *name2 = NULL;

  switch (matemenu_tree_item_get_type (item1))
  {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      name1 = matemenu_tree_directory_get_name (MATEMENU_TREE_DIRECTORY (item1));
      break;
    case MATEMENU_TREE_ITEM_ENTRY:
      return 1;
      name1 = matemenu_tree_entry_get_name (MATEMENU_TREE_ENTRY (item1));
      break;
    default:
      ;
  }
  switch (matemenu_tree_item_get_type (item2))
  {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      name2 = matemenu_tree_directory_get_name (MATEMENU_TREE_DIRECTORY (item2));
      break;
    case MATEMENU_TREE_ITEM_ENTRY:
      return -1;
      name2 = matemenu_tree_entry_get_name (MATEMENU_TREE_ENTRY (item2));
      break;
    default:
      ;
  }
...
populate (Menu *app)
{
  ...
  apps = matemenu_tree_directory_get_contents (app->root);
  if (app->root == MATEMENU_TREE_DIRECTORY(matemenu_tree_get_root_directory ((MateMenuTree*)app->apps)) && app->settings)
  {
    list = g_slist_copy (apps);
    sets = g_slist_copy (matemenu_tree_directory_get_contents (app->settings));
  ...
  if (app->root == matemenu_tree_get_root_directory (app->tree))
  {
    name = matemenu_tree_directory_get_name (app->root);
    gtk_window_set_title (GTK_WINDOW (app->window),
                          matemenu_tree_directory_get_name (app->root));
    label = gtk_label_new ("");
    gtk_box_pack_start (GTK_BOX (vbox), label, FALSE, FALSE, 0);
  }
  else
                 ...
                 Menu *app)
{
  if (event->button == 1)
  {
    if(!GTK_WIDGET_VISIBLE(app->window)) {
      app->root = matemenu_tree_get_root_directory (app->tree);
      populate (app);
    } else {
...
awn_applet_factory_initp (const gchar *name, const gchar *uid, gint panel_id)
{
  ...
  app->apps = MATEMENU_TREE_DIRECTORY(matemenu_tree_lookup ("applications.menu", MATEMENU_TREE_FLAGS_NONE));
  if (!app->apps)
  {
    g_warning ("Unable to find applications.menu");
    return FALSE;
  }
  app->settings = matemenu_tree_get_root_directory (
                  matemenu_tree_lookup ("settings.menu", MATEMENU_TREE_FLAGS_NONE));
  if (!app->settings)
  {
    g_warning ("Unable to find settings.menu");
    //return FALSE;
  }
  app->tree =  (MateMenuTree*)app->apps;
